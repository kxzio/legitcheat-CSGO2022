...
bool ImGui::IsActiveHotkey(ImGuiBind_t* key) {
    if (key != nullptr && key->Key != -1 && key->Type != -1) {
        switch (key->Type) {
        case AlwaysOn:
            return true;
            break;
...
bool ImGui::Hotkey(const char* label, ImGuiBind_t* var, const ImVec2& size_arg)
{
    ...
    const ImRect frame_bb(window->DC.CursorPos + ImVec2(style.ItemInnerSpacing.x, 0.0f) - ImVec2(5, 0), window->DC.CursorPos + size);
    ...
    if (popup_open) {
        SetNextWindowSizeConstraints(ImVec2(size_arg.x, 0.0f), ImVec2(FLT_MAX, CalcMaxPopupHeightFromItemCount(-1)));

        char name[16];
        ImFormatString(name, IM_ARRAYSIZE(name), "##Combo_%02d", ctx->BeginPopupStack.Size); // Recycle windows based on depth
        ...
        if (Selectable("always on", var->Type == AlwaysOn))
            var->Type = 0;
        if (Selectable("always off", var->Type == AlwaysOff))
            var->Type = 1;
        if (Selectable("hold", var->Type == Hold))
            var->Type = 2;
        if (Selectable("toggle", var->Type == Toggle))
            var->Type = 3;
        if (Selectable("release", var->Type == Release))
            var->Type = 4;
    ...
    window->DrawList->AddRect(frame_bb.Min, frame_bb.Max, ImColor(19, 19, 19, 255), style.FrameRounding);

    if (var->Key != 0 && g.ActiveId != id) {
        strcpy_s(buf_display, KeyNames[var->Key]);
    }
    ...
    if (label_size.x > 0)
        ImGui::RenderText(ImVec2(total_bb.Max.x + 5, frame_bb.Min.y + style.FramePadding.y + 1), label);
...
void ImGui::NewFrame()
{
    ...
    g.IO.FontsAdvanced->Locked = true;
    ...
    g.BackgroundDrawList.Clear();
    g.BackgroundDrawList.PushTextureID(g.IO.FontsAdvanced->TexID);
    g.BackgroundDrawList.PushClipRectFullScreen();

    g.ForegroundDrawList.Clear();
    g.ForegroundDrawList.PushTextureID(g.IO.FontsAdvanced->TexID);
    g.ForegroundDrawList.PushClipRectFullScreen();
...
void ImGui::Shutdown(ImGuiContext* context)
{
    ...
    if (g.IO.FontsAdvanced && g.FontAtlasAdvOwnedByContext)
    {
        g.IO.FontsAdvanced->Locked = false;
        IM_DELETE(g.IO.FontsAdvanced);
    }
    g.IO.FontsAdvanced = NULL;
