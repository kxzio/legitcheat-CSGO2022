...
static bool ImGui_ImplDX9_CreateFontsTexture2()
{
	// Build texture atlas
	ImGuiIO& io = ImGui::GetIO();
	unsigned char* pixels;
	int width, height, bytes_per_pixel;
	io.FontsAdvanced->GetTexDataAsRGBA32(&pixels, &width, &height, &bytes_per_pixel);
	...
	// Store our identifier
	io.FontsAdvanced->TexID = (ImTextureID)g_FontTexture;
...
bool ImGui_ImplDX9_CreateDeviceObjects()
{
	...
	if (!ImGui_ImplDX9_CreateFontsTexture())
		return false;
	if (!ImGui_ImplDX9_CreateFontsTexture2())
		return false;
	return true;
}
...
void ImGui_ImplDX9_InvalidateDeviceObjects()
{
	...
	if (g_pVB) { g_pVB->Release(); g_pVB = NULL; }
	if (g_pIB) { g_pIB->Release(); g_pIB = NULL; }
	if (g_FontTexture) { g_FontTexture->Release(); g_FontTexture = NULL; ImGui::GetIO().Fonts->TexID = NULL; ImGui::GetIO().FontsAdvanced->TexID = NULL;} // We copied g_pFontTextureView to io.Fonts->TexID so let's clear that as well.
}
