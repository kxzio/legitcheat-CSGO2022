...
void D3D::Initialise( IDirect3DDevice9 *device ) {
	...
	D3D9Fonts[ MAIN_FONT ] = D3D9DrawManager->add_font( ( "C:\\Windows\\Fonts\\segoeui.ttf" ), 14.f,
		false, false, util::draw::GLYPH_RANGE_LATIN, util::draw::NO_ANTIALIASING | util::draw::BOLD | util::draw::FORCE_AUTO_HINT);
	
	D3D9Fonts[ GROUP_FONT ] = D3D9DrawManager->add_font( ( "C:\\Windows\\Fonts\\segoeuib.ttf" ), 14.f,
		false, false, util::draw::GLYPH_RANGE_LATIN, util::draw::NO_ANTIALIASING | util::draw::MONO_HINTING );

	D3D9Fonts[ TAB_FONT ] = D3D9DrawManager->add_font(( "C:\\Windows\\Fonts\\segoeuib.ttf" ), 21.f,
		false, false, util::draw::GLYPH_RANGE_LATIN, util::draw::NO_ANTIALIASING | util::draw::MONO_HINTING );	
	
	D3D9Fonts[ PIXEL_FONT ] = D3D9DrawManager->add_font(( "C:\\Fonts\\04B03.ttf" ), 8.f,
		false, false, util::draw::GLYPH_RANGE_LATIN, util::draw::NO_ANTIALIASING | util::draw::MONO_HINTING );
...
void D3D::DrawTexture( const vec2& pos, const vec2& size, size_t texture, float alpha ) {
...
void D3D::RectFilled( const vec2 &pos, const vec2 &size, const color &colour ) {
	const auto buf{ D3D9DrawManager->get_buffer( InternalBufferIndex ) };
...
void D3D::TriangleFilled( const vec2 &pos1, const vec2 &pos2, const vec2 &pos3, color &colour ) {
	const auto buf{ D3D9DrawManager->get_buffer( InternalBufferIndex ) };
...
void D3D::Rect( const vec2 &pos, const vec2 &size, color &colour, const float &thickness ) {
	const auto buf{ D3D9DrawManager->get_buffer( InternalBufferIndex ) };
...
void D3D::Circle( const vec2 &center, const float radius, color col, const float thickness, uint32_t parts, float degrees, float start_degree ) {
	const auto buf{ D3D9DrawManager->get_buffer( InternalBufferIndex ) };
...
void D3D::CircleFilled( const vec2 &center, const float radius, color col, const float thickness, uint32_t parts, float degrees, float start_degree ) {
	const auto buf{ D3D9DrawManager->get_buffer( InternalBufferIndex ) };
...
vec2 D3D::GetScreenCenter( ) {
	return GetScreenSize( ) / 2;
}

void D3D::RectFilledLinearGradient( const vec2 &pos, const vec2 &size, color &colour1, color &colour2, bool horizontal ) {
	if ( horizontal ) {
		...
		for ( auto i = 0; i < size.x; ++i ) {
			...
			RectFilled( vec2( pos.x + i, pos.y ), vec2( 1.f, size.y ), color( first, second, third, int( ia * ( colour2.a( ) / 255.f ) ) ) );
		}
	}
	else {
		RectFilled( pos, size, colour1 );
		...
		for ( auto i = 0; i < size.y; ++i ) {
			...
			RectFilled( vec2( pos.x, pos.y + i ), vec2( size.x, 1.f ), color( first, second, third, int( ia * ( (float)colour2.a( ) / 255.f ) ) ) );
		}
	}
}

void D3D::Text( const vec2 &pos, const std::string &text, color &colour, util::draw::font *font, const uint8_t flags ) {
	if ( !font )
		return;
...
void D3D::Text( const vec2 &upos, const std::string &text, color &colour, util::draw::font *font, const uint8_t flags, const EStringAlign align ) {
	if ( !font )
		return;
...
void D3D::Line( const vec2 &pos, const vec2 &pos1, color &colour, float thickness ) {
	const auto buf{ D3D9DrawManager->get_buffer( InternalBufferIndex ) };
...
void D3D::SmoothLine( const vec2 &pos, const vec2 &pos1, bool anti_aliasing, float thickness, color &colour ) {
	const auto buf{ D3D9DrawManager->get_buffer( InternalBufferIndex ) };
...
void D3D::Pixel( const vec2 &pos, color &colour ) {
	const auto buf{ D3D9DrawManager->get_buffer( InternalBufferIndex ) };
...
vec2 D3D::GetTextSize( const std::string &text, util::draw::font *font ) {
	if ( !font )
		return vec2( 0, 0 );
...
std::pair<vec2, vec2> D3D::GetLimit( ) {
	const auto buf{ D3D9DrawManager->get_buffer( InternalBufferIndex ) };
	...
	return std::pair<vec2, vec2>
		( vec2( buf->cur_clip_rect( ).x, buf->cur_clip_rect( ).y ),
			vec2( buf->cur_clip_rect( ).x + buf->cur_clip_rect( ).z, buf->cur_clip_rect( ).y + buf->cur_clip_rect( ).w ) );
}

void D3D::SetLimit( std::pair<vec2, vec2> limit ) {
	const auto buf{ D3D9DrawManager->get_buffer( InternalBufferIndex ) };
